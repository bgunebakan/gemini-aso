...
if __name__ == "__main__":
    ...
    from scripts.lib.reporter import format_competitor_report

    parser = argparse.ArgumentParser(description="Analyze Competitor ASO Strategy")
    parser.add_argument(
    ...
    try:
        ...
        competitors_data = input_data.get("competitors_data", [])
        keywords = input_data.get("keywords", [])
        platform = input_data.get("platform", "apple")
        limit = input_data.get("limit", 10)

        # If no competitors provided, discover them
        if not competitors_data and platform == "apple":
            discovery = DiscoveryEngine()
            competitors_data = discovery.discover_competitors(
                category=category, keywords=keywords, limit=limit
            )

        if not competitors_data:
            results = {"error": "No competitors_data provided and discovery failed"}
        else:
            results = analyze_competitor_set(category, competitors_data, platform)

        if args.format == "markdown":
            print(format_competitor_report(results))
        else:
