...
class MetadataOptimizer:
    ...
    def optimize_subtitle(
        ...
        """
        Optimize Apple App Store subtitle.
        ...
        Returns:
            Optimized subtitle options
        """
        if self.platform != "apple":
            return {"error": "Subtitle optimization only applies to Apple App Store"}
...
if __name__ == "__main__":
    ...
    try:
        ...
        app_info = input_data.get("app_info", {})
        target_keywords = input_data.get("target_keywords", [])

        if not app_info or "name" not in app_info:
            results = {"error": "Missing app_info with app 'name'"}
        else:
            results = optimize_app_metadata(platform, app_info, target_keywords)
