...
if __name__ == "__main__":
    ...
    try:
        ...
        if isinstance(input_data, list):
            results = analyze_keyword_set(input_data)
        elif isinstance(input_data, dict) and "target_keywords" in input_data:
            # Handle format from SKILL.md examples
            kw_data = []
            for kw in input_data["target_keywords"]:
                kw_data.append(
                    {
                        "keyword": kw,
                        "search_volume": 0,  # Estimates would be filled by agent/user
                        "competing_apps": 0,
                        "relevance_score": 0.8,
                    }
                )
            results = analyze_keyword_set(kw_data)
        else:
            results = {
                "error": "Invalid input format. Expected list of keywords or dict with target_keywords."
            }
